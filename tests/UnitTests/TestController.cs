...
using CarCrudApi.Cars.Controllers;
...
public class TestController
{
    ...
    public async Task GetAll_ValidData()
    {
        ...
        Assert.Equal(5, carsAll.carList.Count);
    ...
    public async Task Update_InvalidDate()
    {
        ...
        _command.Setup(repo => repo.UpdateCar(11, update)).ThrowsAsync(new ItemDoesNotExist(Constants.CAR_DOES_NOT_EXIST));

        var result = await _controller.UpdateCar(11, update);
    ...
    public async Task Update_ValidData()
    {
        ...
        _command.Setup(repo=>repo.UpdateCar(5,update)).ReturnsAsync(car);

        var result = await _controller.UpdateCar(5, update);
        var okResult=Assert.IsType<OkObjectResult>(result.Result);
    ...
    public async Task Delete_ItemDoesNotExist()
    {
        ...
        _command.Setup(repo=>repo.DeleteCar(2)).ThrowsAsync(new ItemDoesNotExist(Constants.CAR_DOES_NOT_EXIST));

        var result= await _controller.DeleteCar(2);
        var notfound= Assert.IsType<NotFoundObjectResult>(result.Result);
